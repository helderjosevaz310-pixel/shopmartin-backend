...
async function ensureSchema() {
  ...
  // Garante as colunas de recuperação de password mesmo em bases de dados já existentes
  // (CREATE TABLE IF NOT EXISTS acima não adiciona colunas novas a uma tabela que já existe)
  await pool.query(`
...
/* ============================================================
   RECUPERAÇÃO DE PASSWORD — /api/auth/forgot-password e /reset-password
   ============================================================ */
app.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      // Resposta igual quer o email exista ou não — evita que alguém descubra
      // quais emails têm conta só por tentar recuperar a password deles.
      if (rows.length === 0) {
        return res.json({ message: "Se existir uma conta com este email, foi enviado um link de recuperação." });
      }

      const token = crypto.randomBytes(32).toString("hex");
      const expires = new Date(Date.now() + 60 * 60 * 1000); // válido 1 hora
      ...
      const resetLink = `${process.env.FRONTEND_URL || "https://shopmartin.app"}/?reset_token=${token}`;

      // ⚠️ Ainda não há envio de email configurado neste servidor (precisa de um serviço tipo
      // Resend ou SendGrid). Por agora, devolve o link diretamente na resposta para poderes testar
      // o fluxo. Antes de lançares a app a sério, troca isto por um envio de email real e REMOVE
      // o "resetLink" desta resposta — expô-lo aqui é só para desenvolvimento.
      if (!process.env.RESEND_API_KEY) {
        return res.json({
          message: "Envio de email ainda não configurado — link de teste abaixo (modo desenvolvimento).",
      ...
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: process.env.RESEND_FROM_EMAIL || "ShopMartin <no-reply@shopmartin.app>",
          to: email,
          subject: "Recuperar a tua password — ShopMartin",
          html: `<p>Recebemos um pedido para repor a tua password.</p><p><a href="${resetLink}">Toca aqui para escolher uma nova password</a></p><p>Este link expira em 1 hora. Se não foste tu, ignora este email.</p>`,
        }),
      });

      res.json({ message: "Se existir uma conta com este email, foi enviado um link de recuperação." });
    } catch (e) {
      next(e);
    }
  }
...
app.post(
  ...
  authLimiter,
  [body("token").notEmpty(), body("password").isLength({ min: 8 }).withMessage("A password precisa de pelo menos 8 caracteres")],
  async (req, res, next) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ error: errors.array()[0].msg });
    ...
    try {
      ...
      res.json({ message: "Password alterada com sucesso. Já podes entrar com a nova password." });
    } catch (e) {
      next(e);
    }
  }
