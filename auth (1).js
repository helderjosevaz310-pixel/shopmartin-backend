...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      // Se veio de um link de afiliado, confirma que o código existe antes de o gravar
      let referredBy = null;
      if (ref) {
      ...
      const { rows } = await db.query(
        `INSERT INTO users (name, email, password_hash, affiliate_code, referred_by) VALUES ($1, $2, $3, $4, $5) RETURNING id, name, email, plan, affiliate_code`,
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      // Mensagem genérica propositadamente — não revela se foi o email ou a password que falhou
